...
def fetch_arxiv(config: dict, start_date: date, end_date: date) -> list[Paper]:
    ...
    # Extend back 1 day: arXiv has a 14:00 ET cutoff, so papers announced on
    # `start_date` may have been submitted the previous afternoon.
    arxiv_start = start_date - timedelta(days=1)
    start_str = arxiv_start.strftime("%Y%m%d") + "000000"
...
def _fetch_batch(params: dict) -> list[Paper]:
